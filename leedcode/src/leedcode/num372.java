...
public class num372 {
    public int superPow(int a, int[] b) {
    	...
    	if(a==1337||a==1)
    		...
    		return record.get(modLarge(b,pow)).intValue();
    	}
    	else{
    		long fa=(long)a%1337;
    		long pow=2;
			record.put((long)1,fa%1337);
    		long mod=(fa*fa)%1337;
			record.put(pow, mod);
    		while(mod!=fa%1337){
    			mod=(mod*fa)%1337;
    		...
    		return record.get(modLarge(b,pow-1)).intValue();
    		
    	}
    }
    public long modLarge(int[] done,long be){
    	long mod=1;
    	long result=0;
    	for(int i=done.length-1;i>=0;i--){
    		result=((done[i]*mod)%be+result)%be;
    		mod=((10%be)*mod)%be;
    	}
    	return result;
    }
